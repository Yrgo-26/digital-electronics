...
for file in **/*.md; do
    ...
    while IFS= read -r target; do
        ...
        checked=$((checked + 1))
        if [ ! -e "$resolved" ]; then
            # A solutions appendix is git-ignored until the lecture has been given, so in a fresh
            # clone a link to it has no target yet. That is unpublished, not broken: git itself
            # says which, since check-ignore works on a path that does not exist.
...
echo "Link check: $((checked - unpublished)) relative link(s) resolve; $unpublished point to" \
     "unpublished (git-ignored) files."
